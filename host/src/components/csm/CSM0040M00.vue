<script setup>
  ...
  const fields = reactive([
    'NO',
    { key: 'athCd', label: 'Authority Code' },
    { key: 'athName', label: 'Authority Name' },
    { key: 'athPtrnDstcd', label: 'Authority Type' },
  ...
  const athPtrnDstcdList = reactive([
    { name: 'Mother To Headquarters Division', value: '1' },
    // { name: 'Overseas Corporation Headquarters Division ', value: '2' },
    { name: 'Business Branch ', value: '3' },
    { name: 'Others', value: '9' },
  ]);
  ...
  const fetchList = () => {
    apiCommon.callApi(`/csm/selectListAthCdMgt`, 'post', inqParam.value, fetchListCallback);
  };
  ...
  // 거래 실행 전 유효성 검사
  const validateExe = () => {
    const { athCd, athName, athPtrnDstcd, useYn } = insParam;

    if (tranType.value !== '1' && athCd === '') errorMessage.value = 'Missing equired value (Authority Code)';
    if (athName === '') errorMessage.value = 'Missing equired value (Authority Name)';
    if (athPtrnDstcd === '') errorMessage.value = 'Missing equired value (Authority Type)';
    if (useYn === '') errorMessage.value = 'Missing equired value (Used)';
  ...
  // 거래 실행
  const execBtn = () => {
    const exeUris = ['insertAthCdMgt', 'updateAthCdMgt', 'deleteAthCdMgt'];
    const uri = exeUris[parseInt(tranType.value) - 1];
    const validate = validateExe();

    if (tranType.value === '1') {
      insParam.applicationType =
        sessionInfo.applicationName && sessionInfo.applicationName === 'loankbpbonl' ? 'L' : 'I';
    }
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>
              <b-form-input type="text" v-model="inqParam.athCd" maxLength="6" />
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <div class="con_area">
    ...
    <div class="boardlist over">
      ...
      >
        ...
        <template #cell(athPtrnDstcd)="row">
          <b-form-select
            v-model="row.item.athPtrnDstcd"
            v-select-mix
            :options="athPtrnDstcdList"
            readonly
            value-field="value"
            text-field="name"
          />
    ...
    <div class="boxarea">
      ...
      <div v-bind:class="[{ boardwrite: true }, { disabled: tranType === '3' }]">
        <table>
          <tbody>
            <tr>
              <th class="req">Authority Code</th>
              <td>
                <b-form-input type="text" v-model="insParam.athCd" disabled class="req" />
              </td>
              <th class="req">Authority Name</th>
              <td>
                <b-form-input type="text" v-model="insParam.athName" class="req" />
              </td>
              <th class="req">Authority Type</th>
              <td>
                <b-form-select
                  v-select-mix
                  v-model="insParam.athPtrnDstcd"
                  :options="athPtrnDstcdList"
                  value-field="value"
                  text-field="name"
                  class="req"
