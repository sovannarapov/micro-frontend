...
var OZUtil = {
	...
	removeViewerID : function(OZViewerID)
	{
		...
		if(OZMap)
		{
			...
			// Clean up ViewerIDs
			// indexOf 함수를 IE9 버전부터 지원 하여 아래와 같이 변경함.
			var length = OZUtil.OZOZViewerIDs.length;
			for (var i = 0; i < length; i++) {
				if(OZViewerID == OZUtil.OZOZViewerIDs[i])
				{
					OZUtil.OZOZViewerIDs.splice(i, 1);
					break;
				}
			}
...
*/				
	...
	GetELConnection : function() {
		
		var CONNECTION = null;
		var cur_protocolName = window.location.protocol;
		var url;
		if (cur_protocolName == 'https:') {
			url = 'wss://127.0.0.1:' + _sslport +'/ozelservice';
		} else {
			url = 'ws://127.0.0.1:' + _port +'/ozelservice';
		}
		try {
			CONNECTION = new WebSocket(url);
		} catch (e) {
	...
	getRequestELURL : function(){
		
		var cur_protocolName = window.location.protocol;
		var url = "";
		if (cur_protocolName === 'https:') {
			url = "https://127.0.0.1:" + _sslport +"/ozelservice";
		} else {
			url = "http://127.0.0.1:" + _port +"/ozelservice";
		}
		return url;
		
	},
	
	startEL : function(id, elParam) {
		var ELMap = OZUtil.OZEL_Map[id];
		if(ELMap)
		{
			console.log("EL already running");
			return;
		}
		
		var verNumber = parseInt(OZUtil.GetIEVersion(), 10);
		if (verNumber > 10 || verNumber == -1) {
			OZUtil.startEL_WS(id, elParam);
		} else {
			OZUtil.startEL_HTTP(id, elParam);
		}
	},
	
	startEL_WS : function(id, elParam) {
		var CONNECTION = OZUtil.GetELConnection();
		...
		CONNECTION.onopen = function() {
			elParam.ID = id;
			CONNECTION.send("REL:" + JSON.stringify(elParam));
		};
		...
		CONNECTION.onmessage = function(e) {
			...
			if (cmdtype == "RET") {
				...
				if (jsonInfo.result == "OK") {
					OZUtil.OZEL_Map[id] = [jsonInfo.elkey,false];
					OZUtil.OZEL_IDs.push(id);					
				} else {
					console.log(jsonInfo.reason);
				}
	...
	startEL_HTTP : function(id, elParam) {
		
		elParam.type  = "REL";
		elParam.ID = id;
		
		var url  = OZUtil.getRequestELURL();
		if(OZUtil.GetIEVersion() < 9 && OZUtil.GetIEVersion() != -1)
		{
			var xdr = OZUtil.createXDRHttpRequest();
			
			xdr.open("GET", url + encodeURIComponent(JSON.stringify(elParam)), true);

			 xdr.onerror = function() {
				console.log("fail to send rel parameter. (error)");
			};
			xdr.ontimeout = function () { 
				console.log("fail to send rel parameter. (timeout)");
			};
			
			xdr.onload = function () { 
				var jsonInfo = JSON.parse(xdr.responseText);
				if (jsonInfo.result == "OK") {
					OZUtil.OZEL_Map[Args_ID] = [jsonInfo.elkey,false];
					OZUtil.OZEL_IDs.push(Args_ID);		
				} else {
					console.log(jsonInfo.reason);
				}
			};
			...
			window[callbackMethod] = (function (Args_ID, Args_elParam, jsonInfo) {
				if (jsonInfo.result == "OK") {
					OZUtil.OZEL_Map[Args_ID] = [jsonInfo.elkey,false];
					OZUtil.OZEL_IDs.push(Args_ID);		
				} else {
			...
			}).bind(null, id, elParam);
			
			var callbackErrorMethod = callbackMethod + "_error";
			window[callbackErrorMethod] = function(e) {
				console.log("fail to send rel parameter");
			};
			
			var script = document.createElement('script');
			script.onerror = window[callbackErrorMethod];
			script.src = url + encodeURIComponent(JSON.stringify(elParam)) + '?callback='+callbackMethod;
			document.body.appendChild(script);
		}
	},
	...
	closeEL : function(id) {
		var verNumber = parseInt(OZUtil.GetIEVersion(), 10);
		if (verNumber > 10 || verNumber == -1) {
			OZUtil.closeEL_WS(id);
		} else {
			OZUtil.closeEL_HTTP(id);
		}
	},
	closeEL_WS : function(id) {
		var CONNECTION = OZUtil.GetELConnection();
		if(_isHttpMode){
			OZUtil.closeEL_HTTP(id);
			return;
		...
		CONNECTION.onopen = function() {
			...
			if(ELMap)
			{
				...
				var packet = new Object();
				packet.id = id;
				packet.elkey = ELMap[0];
				CONNECTION.send("EEL:" + JSON.stringify(packet));
				CONNECTION.close();
				
				// Clean up NScreenAD IDs
				var idx = OZUtil.OZEL_IDs.indexOf(id);
				if(idx > -1)
				{
					OZUtil.OZEL_IDs.splice(idx, 1);
				}
				
				// Clean up NScreenAD map 
				delete OZUtil.OZEL_Map[id];
			}
		};
		// when the connection is closed by the server
		CONNECTION.onclose = function(e) {
	...
	closeEL_HTTP : function(id) {
		...
		if(ELMap)
		{
			...
			// 종료 이벤트 전달
			OZUtil.Cmd_SendELPacket(JSON.stringify(packet));
	...
	closeAllEL : function() {
		var verNumber = parseInt(OZUtil.GetIEVersion(), 10);
		if (verNumber > 10 || verNumber == -1) {
			OZUtil.closeAllEL_WS();
		} else {
			OZUtil.closeAllEL_HTTP();
		}
		
		var arr = OZUtil.OZEL_IDs;
		if(arr != null)
		{
			for(var i =0; i < arr.length; i ++)
			{
				var obj  = OZUtil.OZEL_Map[arr[i]];
				delete OZUtil.OZEL_Map[arr[i]];
			}	
		}
		
		if(OZUtil.OZEL_IDs.length > 0)
		{
			OZUtil.OZEL_IDs.splice(0, OZUtil.OZEL_IDs.length);
		}
	},
	
	closeAllEL_WS : function() {
		var CONNECTION = OZUtil.GetELConnection();
		if(_isHttpMode){
			OZUtil.closeAllEL_HTTP();
			return;
		...
		CONNECTION.onopen = function() {
			var packet = new Object();
			packet.type = "CEL";
			CONNECTION.send("CEL:" + JSON.stringify(packet));
			CONNECTION.close();		
		};
		// when the connection is closed by the server
		CONNECTION.onclose = function(e) {
		...
		// Log errors
		CONNECTION.onerror = function(e) {
			console.log("error in EL all close");
		};
		// Log messages from the server
		CONNECTION.onmessage = function(e) {
	...
	closeAllEL_HTTP : function() {
		var packet = new Object();
		packet.type = "CEL";
		OZUtil.Cmd_SendELPacket(JSON.stringify(packet));
	},
	
	Cmd_SendELPacket : function(packetstr) {
		...
		if(OZUtil.GetIEVersion() < 9 && OZUtil.GetIEVersion() != -1)
		{
			...
			var callbackErrorMethod = callbackMethod + "_error";
				window[callbackErrorMethod] = function(e) {
					console.log("fail to send EL packet");
				};
	...
	ReleaseEL : function(){
		OZUtil.OZEL_IDs = null;
		OZUtil.OZEL_Map = null;
	},
